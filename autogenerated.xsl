...
<xsl:transform xmlns="http://www.w3.org/1999/xhtml"
   ...
   <!--@temp:replace-id='d1e364' is actually: /h:html/h:body/*[@data-type='chapter']/*[@data-type='page']-->
   ...
   <!--@temp:replace-id='d1e379' is actually: /h:html/h:body/*[@data-type='chapter']/*[@data-type='page']/*[@data-type='document-title']-->
   ...
   <xsl:template mode="ANNOTATE_MODE"
      ...
      <xsl:copy inherit-namespaces="no">
         <xsl:attribute name="temp:replace-id">d1e396</xsl:attribute>
         <xsl:apply-templates mode="ANNOTATE_MODE" select="@*|node()"/>
      </xsl:copy>
   </xsl:template>
   <!--@temp:replace-id='d1e396' is actually: /h:html/h:body/*[@data-type='page'][@class][func:hasClass(@class, 'preface')]-->
   <xsl:template mode="EXPAND_MODE" match="*[@temp:replace-id = 'd1e396']">
         <!--r:this-->
      <xsl:copy>
         <xsl:apply-templates select="@*"/>
         <!--r:children selector=""-->
   ...
   <xsl:template mode="NUMBER_MODE" match="*[@temp:replace-id = 'd1e396']">
      <xsl:copy>
         <xsl:apply-templates mode="NUMBER_MODE" select="@*"/>
         <xsl:if test="r:link-text/node()">
            <xsl:attribute name="temp:linktext">
   ...
   <xsl:template mode="ANNOTATE_MODE"
      ...
      <xsl:copy inherit-namespaces="no">
         <xsl:attribute name="temp:replace-id">d1e403</xsl:attribute>
         <xsl:apply-templates mode="ANNOTATE_MODE" select="@*|node()"/>
      </xsl:copy>
   </xsl:template>
   <!--@temp:replace-id='d1e403' is actually: /h:html/h:body/*[@data-type='page'][@class][func:hasClass(@class, 'preface')]/*[@data-type='document-title']-->
   <xsl:template mode="EXPAND_MODE" match="*[@temp:replace-id = 'd1e403']">
      <inject-element inject-name="h1" inject-data-type="document-title">
         <inject-element inject-name="span" inject-class="os-text">
                  <!--r:children selector=""-->
            <xsl:apply-templates mode="EXPAND_MODE" select="node()"/>
   ...
   <xsl:template mode="NUMBER_MODE" match="*[@temp:replace-id = 'd1e403']">
      <xsl:copy>
         <xsl:apply-templates mode="NUMBER_MODE" select="@*"/>
         <xsl:if test="r:link-text/node()">
            <xsl:attribute name="temp:linktext">
