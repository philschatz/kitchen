...
<xsl:template match="r:root">
    <t:transform expand-text="yes" version="3.0" exclude-result-prefixes="h r xs fn temp func">
...
<xsl:template match="r:replace">
    ...
    <xsl:choose>
        ...
        <xsl:otherwise>
            <t:template mode="MOVE_MODE" match="{$matchString}">
                <t:variable name="this" select="."/>
                <t:copy>
                    <t:apply-templates mode="MOVE_MODE" select="@*|node()">
                        <t:with-param tunnel="yes" name="nearestReplacerContext" select="$this"/>
                    </t:apply-templates>
                </t:copy>
            </t:template>
        </xsl:otherwise>
