...
if [[ -d "${XSLT_COVERAGE_HOME}" ]] && [[ "${XSPEC_HOME}" != '' ]]; then
    ...
    ${XSLT_COVERAGE_HOME}/xslt-coverage.bash to-xslt.xsl ./recipe.xml ./coverage/coverage-one.json autogenerated.xsl
    ${XSLT_COVERAGE_HOME}/xslt-coverage.bash autogenerated.xsl "${input_file}" ./coverage/coverage-two.json "${output_file}"

    node ${XSLT_COVERAGE_HOME}/mergeCoverageFiles.js coverage/coverage-one.json coverage/coverage-two.json > coverage/coverage-final.json
    rm coverage/coverage-one.json coverage/coverage-two.json
